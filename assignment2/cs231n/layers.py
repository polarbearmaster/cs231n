...
def batchnorm_backward(dout, cache):
    ...
    # dx_hat = dout * gamma
    # d_var = np.sum(dx_hat * (x - sample_mean),axis=0) * (-1/2) * ((sample_var + eps) ** (-1.5))
    # d_mean = np.sum(dx_hat * (-1/np.sqrt(sample_var + eps)),axis=0) + np.sum(-2*(x-sample_mean)/N * d_var,axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx_hat = dout * gamma
    d_var = np.sum(dx_hat * (x - sample_mean),axis=0) * (-1/2) * ((sample_var + eps) ** (-1.5))
    d_mean = np.sum(dx_hat * (-1/np.sqrt(sample_var + eps)),axis=0) + np.sum(-2*(x-sample_mean),axis=0)/N * d_var
    dx = dx_hat / np.sqrt(sample_var + eps) + d_var * 2*(x-sample_mean)/N + d_mean/N
    dgamma = np.sum(dout * x_hat, axis = 0)
    dbeta = np.sum(dout, axis = 0)
    ###########################################################################
